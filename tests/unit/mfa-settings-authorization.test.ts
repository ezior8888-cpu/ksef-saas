...
const mocks = vi.hoisted(() => ({
  ...
  reauth: vi.fn(), generate: vi.fn(), remove: vi.fn(), audit: vi.fn(),
...
import {
  enrollTotpAction, verifyTotpEnrollmentAction, regenerateRecoveryCodesAction, unenrollTotpAction,
...
describe.each([
  ...
  it('still requires the correct password after verified MFA', async () => {
    ...
    await expect(run()).resolves.toEqual({ ok: false, error: 'invalid_password' });
    expect(mocks.generate).not.toHaveBeenCalled();
    expect(mocks.remove).not.toHaveBeenCalled();
    expect(mocks.unenroll).not.toHaveBeenCalled();
  });
...
it('regenerates codes only for the authoritative current user', async () => {
  await expect(regenerateRecoveryCodesAction('fixture-password')).resolves.toEqual({ ok: true, recoveryCodes: ['synthetic-code'] });
  expect(mocks.generate).toHaveBeenCalledWith(user.id);
});
...
it('generates initial recovery codes only after successful TOTP verification', async () => {
  mocks.challenge.mockResolvedValue({ data: { id: 'challenge-fixture' }, error: null });
  mocks.verify.mockResolvedValue({ error: { message: 'bad code' } });
  await expect(verifyTotpEnrollmentAction('factor-fixture', '123456')).resolves.toEqual({ ok: false, error: 'verify_failed' });
  expect(mocks.generate).not.toHaveBeenCalled();
  mocks.verify.mockResolvedValue({ error: null });
  await expect(verifyTotpEnrollmentAction('factor-fixture', '123456')).resolves.toMatchObject({ ok: true });
  expect(mocks.generate).toHaveBeenCalledWith(user.id);
});
