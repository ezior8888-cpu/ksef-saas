...
## 2026-09-24 — trwałe zwroty Stripe i uzgodnienie wyniku (CYB-F03-25)

**Dyspozycja i baza:** Igor zatwierdził dalszą pracę po publikacji [roboczego PR #29](https://github.com/ezior8888-cpu/ksef-saas/pull/29). Jego dokładny HEAD `ee8ea6622a24be9a250fdd7503441a3258c037a3` miał 7/7 kontroli CI/Security PASS; PR pozostaje draft/open, bez merge i wdrożenia. Osobna lokalna gałąź `codex/security-refund-idempotency` wychodzi z tego HEAD. Root checkout i zmiany użytkownika pozostały nietknięte.

**Problem:** dawny kod admina wysyłał `refunds.create` przed lokalnym zapisem i bez klucza idempotencji. Utrata odpowiedzi lub awaria bazy mogła pokazać nieudaną operację mimo zwrotu przyjętego przez Stripe i zachęcać do kolejnej próby. Spóźniony webhook potrafił odtworzyć status `succeeded`, a panel sumował także zwroty `pending`/`failed`. Ustalenia pochodzą z przeglądu kodu, migracji i testów hermetycznych; nie stwierdzono incydentu na produkcji.

**Zmiana:** [pełny zakres, preflight i runbook](ZWROTY-STRIPE-UZGODNIENIE-2026-09-24.md). Migracja `00075` definiuje trwały claim przed Stripe, stały klucz idempotencji, złożone relacje payment–refund–tenant i odrzuca cały spóźniony upsert płatności podczas lub po zwrocie. Historyczne rekordy refundów dostają stan do ręcznego uzgodnienia; złożony FK dla starszej tabeli jest `NOT VALID` i chroni nowe zapisy. Kod blokuje powtórną próbę przy wyniku niepewnym, wymaga potwierdzonego `succeeded` i trwałego zapisu, odróżnia równoległe `processing` od rzeczywistego uzgodnienia, zatrzymuje skutki uboczne spóźnionych webhooków i opóźniony job faktury VAT. Panel liczy wyłącznie potwierdzone zwroty; `processing` ponad 15 minut wyświetla potrzebę uzgodnienia, a monitor wysyła licznikowy alert bez danych klienta. Commit kodu, migracji, testów i runbooka: `caff1ebbaffef154df19c4009696ac32cc4b2849`.

**Weryfikacja:** 152 pliki / 2767 testów Vitest PASS; 66 testów XML i 67 testów narzędzi bezpieczeństwa PASS; pełny typecheck PASS w izolowanej konfiguracji z lokalnym aliasem typu jsdom. Pełny lint: 0 błędów / 29 wcześniejszych ostrzeżeń, zmienione pliki bez nowych ostrzeżeń. Pełny Next.js build --webpack PASS, 82/82 stron, w kopii 1310 plików bez `.env` i sekretów, bez standalone/Dockera; odcisk kopii i kodu przed dopisaniem tego wpisu identyczny `db42db6c911ca20d3b4703e9cf709f5d365364c8dfa23491e1eb797bd54bedb4`. Gitleaks 8.30.1 na 17 zmienionych plikach wraz z dziennikiem: 0 trafień; `git diff --cached --check` PASS. Dowody lokalne: TEMP/faktflow-refund-build-37bbef2e983a4761b4531057057f59eb oraz TEMP/faktflow-refund-final-scan-3fAY1S. Niezależny przegląd wykrył i doprowadził do naprawy wyścigu webhooka, błędnej sumy pending i niespójności tenantów.

**Status i odbiór:** pakiet jest lokalny; nowe kontrole GitHub CI/Security jeszcze go nie obejmują. Nie wykonano migracji, wywołania Stripe, testu prawdziwego PostgREST, merge ani wdrożenia. Bartek ma sprawdzić historię zwrotów i granty, wykonać migrację `00075` na osobnej bazie, przetestować Stripe test mode oraz przerwę administracyjnych zwrotów przez cały rollout webu/workerów. Po rozliczeniu historycznych mismatch powinien zwalidować `stripe_refunds_payment_tenant_fk` i potwierdzić stan `pg_constraint.convalidated`. Na produkcji potrzebna jest kontrola operacji starszych niż 15 minut i procedura ręcznego uzgodnienia bez automatycznego ponowienia. Oddzielny wyścig event-claim webhooka i okno między końcowym odczytem joba VAT a zwrotem nadal wymagają dalszego projektu/testu transakcyjnego; żadna zielona symulacja nie stanowi gwarancji Stripe live.

## 2026-09-24 — odbiór webhooków Stripe i wiązanie billingu (CYB-F03-26)

**Dyspozycja i baza:** Igor polecił kontynuację. Osobny worktree i gałąź `codex/security-stripe-webhook-claim` startują z dokładnego HEAD roboczego PR #32, `52a0071671a5a3bac1af0f96e020250e564f8889`. PR #32 pozostaje punktem zależnym, a jego kontrole nie są wynikiem nowego pakietu. Główny checkout i zmiany użytkownika pozostały nietknięte.

**Scenariusz i źródło:** przegląd route, mappera, joba i migracji wykazał wyścig dwóch dostaw tego samego `evt_*`, brak sprawdzanej finalizacji, ciche pominięcie faktury przed subskrypcją i możliwość nadpisania anulowanego stanu. W `00002` rola `authenticated` ma tabelowy UPDATE `tenants`, a polityka ownera w `00037` nie ogranicza kolumn; właściciel mógł zmienić `stripe_customer_id`. Dwa równoległe Checkout mogły nadpisać przypisanie. Job maila o nieudanej płatności mógł wysłać wezwanie po późniejszym sukcesie. To ustalenia kodu, migracji i hermetycznych testów, bez stwierdzenia incydentu produkcyjnego.

**Wykonana zmiana:** commit `2db95811902762dd3e228ff0580db631e8a078a1` obejmuje 24 pliki. [Szczegóły, licznikowe kontrole i plan odbioru](WEBHOOK-STRIPE-ODBIOR-2026-09-24.md). `00076` dodaje atomowy claim/finalizację webhooka tylko dla `service_role`, token właściciela i bezpieczne ponowienie wyłącznie po błędzie udowodnionym przed skutkami ubocznymi; stany niepewne nie są automatycznie odtwarzane. `00077` chroni terminalne stany płatności i subskrypcji, relacje firma–płatność–subskrypcja, niezmienność przypisanego Stripe Customer także dla starszej instancji aplikacji oraz indeksy alarmów. Złożony FK jest `NOT VALID` i nie naprawia historii. Checkout/portal sprawdzają zgodność Customer w Stripe z firmą, a pierwszy zapis używa atomowego warunku NULL. Handler obsługuje obie wersje referencji subskrypcji w fakturze, nie gubi faktury dostarczonej wcześniej od subskrypcji, sprawdza utrwalony status przed skutkami, alarmuje o zaległych claimach i usuwa dwa zdarzenia kolejki bez odbiorców. Job windykacyjny przed mailem sprawdza firmę, płatność i bieżącą fakturę w Stripe; chwilowy błąd odczytu zostawia `sending` do licznikowego alertu.

**Weryfikacja:** finalnie 160 plików / 2850 testów Vitest PASS; 66 testów XML PASS; 67 testów narzędzi bezpieczeństwa PASS. Izolowany TypeScript PASS; pełny lint 0 błędów / 29 wcześniejszych ostrzeżeń, zmienione pliki bez nowego błędu. Pełny Next.js `build --webpack` PASS, 82/82 stron, w kopii TEMP bez plików `.env`, bez standalone/Dockera i bez usług zewnętrznych. Wszystkie 24 pliki commitu były bajtowo identyczne z kopią buildu (1322 pliki źródłowe, odcisk `46bfb832bbb2082399d2984f854432bde3a6f0160917397777e54471422c43cd`). Gitleaks 8.30.1: 24 pliki, 0 trafień; `git diff --cached --check` PASS. Dowody lokalne: TEMP/faktflow-webhook-verified-build-2117af1430a74d8d9823e6965fda2346 i faktflow-webhook-final-scan-f2d401a6ed884f97893d27f5ff5f7a65. Niezależne przeglądy wykryły i doprowadziły do poprawienia wyścigu Customer, spóźnionego maila i błędnej klasyfikacji chwilowego błędu Auth.

**Granice:** nie wykonano SQL, migracji, prawdziwego Stripe, Resend, PostgREST, merge ani wdrożenia. Brak testu rzeczywistych danych historycznych i dostarczenia alarmu. Starszy nieterminalny snapshot subskrypcji (`active`/`past_due`) nadal może nadpisać nowszy; job faktury VAT i refund mają osobny wyścig; zewnętrzny enqueue nie ma outboxu; ostatni odczyt przed mailem nie jest atomowy z Resend. Stany webhooka `processing`/`failed` i powiadomienia `sending` wymagają kontrolowanego uzgodnienia, bez resetu i automatycznego replay. F03 pozostaje otwarta.

**Bartek — odbiór:** potwierdzić zastosowanie `00075`, realne granty, historyczne statusy, zgodność Customer–firma i metadanych Stripe, kopię oraz próbę restore. Na kopii zmierzyć DDL i sprawdzić dwa tenanty, dwa równoległe Checkout, obie kolejki, idempotencję i skutki awarii. W oknie wdrożenia wstrzymać endpoint webhooka i nowe Checkout, opróżnić stare instancje, dopiero potem zastosować `00076` i `00077` oraz wymienić web/worker. Po ręcznym rozliczeniu starych niespójności zwalidować złożony FK; potwierdzić alarmy i procedurę uzgadniania ze Stripe/Resend. Żadnej operacji na serwerze nie wykonał Codex.

**Stan wydania:** dwa commity lokalne po dopisaniu tego wpisu; nowy kod nie ma jeszcze wyników GitHub CI/Security ani publicznego PR. Publikacja, merge i produkcyjne uruchomienie są osobnymi krokami. Kolejny pakiet powinien dodać monotoniczny mirror subskrypcji i trwały protokół skutków ubocznych.

## 2026-09-24 — spójny stan subskrypcji i plan faktury VAT (CYB-F04-26)

**Dyspozycja i baza:** Igor polecił kontynuację. Poprzedni pakiet 2db9581 + 43c2124 jest opublikowany jako [roboczy PR #34](https://github.com/ezior8888-cpu/ksef-saas/pull/34), zależny od #32. Dokładny HEAD 43c2124 miał 7/7 kontroli CI/Security PASS i zero otwartych alertów CodeQL dla gałęzi. PR nadal jest draft/open, bez merge. Nowa izolowana gałąź codex/security-stripe-state-order wychodzi z 43c2124; główny checkout i zmiany użytkownika pozostały nietknięte.

**Scenariusz:** Stripe nie gwarantuje kolejności webhooków. Sam atomowy claim zdarzenia nie zapobiegał nadpisaniu bieżącego planu starszym, innym subscription.updated. Nieznana cena wpadała do planu miesięcznego. Równoległe Checkout mogły tworzyć kolejne subskrypcje. Opóźniony job faktury VAT odczytywał aktualny plan zamiast planu opłaconej faktury; kolizja skróconego numeru wewnętrznego mogła skojarzyć dwie różne faktury Stripe z jednym dokumentem VAT. To ustalenia ze statycznego przeglądu kodu, migracji i testów, bez stwierdzonego incydentu na produkcji.

**Zmiana:** commit kodu, migracji, testów i [runbooka odbioru](STRIPE-SUBSKRYPCJE-ODBIOR-2026-09-24.md): cd550ca. Migracja 00078 wprowadza krótką dzierżawę per subskrypcja, token i fencing oraz RPC atomowego zapisu aktualnego snapshotu Stripe; odbiera service_role bezpośredni zapis do tabeli subscriptions. Cztery zdarzenia subskrypcji odczytują aktualny obiekt Stripe. Mapper odrzuca nieznaną cenę/status i subskrypcję spoza modelu jednej pozycji. Checkout sprawdza lokalną bazę i Stripe oraz używa klucza idempotencji na godzinę. Job VAT bierze plan z zapisanego snapshotu konkretnej opłaconej faktury, z kontrolą ID, kwoty, waluty i pojedynczej pozycji bez proraty. Ponowne użycie faktury po kolizji numeru wymaga zgodności pełnego Stripe invoice ID, nabywcy, kwoty i zapisanej pozycji; obcy lub niepełny rekord zatrzymuje automat. Format numeru pozostaje bez zmian, aby nie tworzyć duplikatów przy starszych retry.

**Weryfikacja:** po ostatniej zmianie 165 plików / 2929 testów Vitest PASS; 66 XML i 67 testów narzędzi bezpieczeństwa PASS. Izolowany TypeScript PASS, lint zmienionych plików bez błędów i ostrzeżeń. Pełny Next.js build --webpack PASS, 82/82 stron; bez standalone/Dockera i zewnętrznych usług. Niezależny przegląd nie znalazł nowego błędu P1/P2 w integracji i wskazał kolizję numeru, naprawioną przed tym zapisem. Gitleaks 8.30.1 na dodanych liniach i dokładnym commicie cd550ca: brak trafień; git diff --cached --check PASS. Skan całych 19 plików zgłosił jeden wcześniejszy, niezmieniony przykładowy klucz w lib/stripe/client.ts:24; przejrzano diff tego pliku. Tymczasowa konfiguracja testowa została usunięta z worktree.

**Granice i odbiór:** commit jest lokalny, bez wyników CI/Security dla nowej gałęzi. SQL i migracji nie wykonano; brak testu prawdziwego PostgREST, Stripe, KSeF, Resend, merge i wdrożenia. Trwały claim Checkout poza granicą godziny, transakcja refund–faktura, outbox webhooków oraz pełne unikalne powiązanie faktury VAT po Stripe invoice ID pozostają osobnymi zadaniami. Obecny generator wylicza stałe 23% VAT z gross; poprawność księgową i miejsce opodatkowania musi potwierdzić człowiek przed live. Płatności bez pełnego snapshotu i przypadki proraty wymagają uzgodnienia.

**Bartek — warunek odbioru:** potwierdzić PR #32/#34 i 00075–00077, kopię oraz próbę restore; na kopii sprawdzić granty i DDL 00078, historyczne subskrypcje/płatności oraz FK NOT VALID. W Stripe test mode sprawdzić Price ID, PLN i okresy, odwrócone webhooki, równoległe dostawy, wygasłą dzierżawę, obcego Customer, wiele pozycji i plan zmieniony po płatności. Wspólnie z księgowym rozliczyć już wystawione dokumenty i zasadę 23% VAT. Rollout wymaga wstrzymania webhooka i nowych Checkout, zakończenia starych instancji, zastosowania 00078 przez właściciela, wymiany webu/workerów, kontroli grantów i dopiero ponownego otwarcia wejścia. Po 00078 samo cofnięcie obrazu aplikacji nie przywraca dawnego zapisu tabeli. Szczegóły i odczytowe zapytania w runbooku.

**Stan wydania:** kod zweryfikowany lokalnie, migracja nieuruchomiona, nowy PR nieopublikowany. Zielone kontrole #34 potwierdzają tylko poprzedni HEAD, nie ten commit.

## 2026-09-25 — atomowa granica faktury VAT i zwrotu Stripe (CYB-F03-28)

**Dyspozycja i stan wyjściowy:** Igor polecił kontynuować bezpieczeństwo billingu. Osobny worktree i gałąź codex/security-refund-vat-atomicity startują z dokładnego HEAD PR #35, 5be027cf234eb714ce4b46f19a48cc795b25457b. Igor potwierdził, że skierowanie PR #34/#35 na main i oznaczenie jako gotowe było celowe. Oba PR są w GitHubie otwarte i konfliktowe; sam status GitHuba nie mówi, jaki obraz działa na Coolify. Igor doprecyzował, że 00078 jest scalona tylko do release/2026-09-25-maslo, a wykonanie na db-1 nie jest potwierdzone. Do uzyskania dowodu traktujemy 00078 jako niewdrożoną. Główny checkout i własne zmiany użytkownika pozostały nietknięte.

**Scenariusz i źródło:** statyczny przegląd joba VAT, admin refundu, webhooka i migracji wykazał lukę między osobnym zapisem nagłówka, pozycji i linku faktury a claimem zwrotu. Równoległe operacje mogły dopuścić jednocześnie fakturę i zwrot. Brak autorytatywnego paid_at pozwalał przy ponowieniu po zmianie miesiąca nadać tej samej płatności drugi numer. Po zapisie linku i awarii przed enqueue do KSeF stary retry mógł cicho pominąć wysyłkę. Nie stwierdzono rzeczywistego incydentu produkcyjnego.

**Zmiana lokalna:** commit e4b9a16c7e0e52bb989ec76ae57d9027dc44cdc1. Migracja 00079 dodaje trwałe pełne Stripe invoice ID, dwa RPC z tą samą blokadą wiersza płatności, atomowe utworzenie faktury/pozycji/linku, odmowę automatycznego zwrotu przy fakturze i ochronę dokumentu/linku przed bezpośrednią zmianą. Historyczne faktury bez pewnego pełnego ID nie są backfillowane automatycznie. Skany starszych notes/fa3_data ograniczono do firmy operatora po niezależnym przeglądzie ryzyka blokady cudzego zwrotu. Kod webhooka wymaga podpisanej daty zapłaty i potwierdza ją w DB; job nie bierze już bieżącego czasu. Admin refund zgłasza claim przez RPC przed Stripe, a brak konfiguracji operatora zatrzymuje automat. Niepewny enqueue do KSeF jest widoczny przez znacznik dopiero po sendEvent oraz licznikowy alert po 15 minutach, bez automatycznego ponowienia. [Runbook Bartka i granice](VAT-STRIPE-ZWROTY-ODBIOR-2026-09-25.md).

**Weryfikacja:** 166 plików / 2955 testów Vitest PASS z izolowanym aliasem lokalnego jsdom; po końcowej zmianie parametru operatora 4 pliki / 46 istotnych testów PASS. 66 testów XML, 67 testów narzędzi bezpieczeństwa PASS. Pełny lint 0 błędów / 26 wcześniejszych ostrzeżeń; zmienione pliki bez błędów. TypeScript PASS w izolowanej konfiguracji z typami jsdom; zwykły pnpm typecheck w tym worktree zgłasza jedynie dwa wcześniejsze braki modułu jsdom w testach UI. Pełny Next build --webpack PASS, 82/82 stron, w izolowanym worktree bez .env.local; nie był to obraz Docker/serwer. Gitleaks 8.30.1: 15 zmienionych plików, 0 trafień; git diff --cached --check PASS. Statyczny niezależny przegląd SQL po poprawkach nie znalazł pewnego blokera, lecz nie zastępuje wykonania na kopii bazy.

**Granice i właściciel odbioru:** Codex nie uruchomił migracji, SQL, prawdziwego Stripe/KSeF/PostgREST, nie scalał PR i nie wdrażał na serwer. 00079 należy stosować dopiero po udowodnieniu 00075–00078 na db-1, kopii/restore i próbach na kopii; stary web/worker trzeba wyłączyć, bo 00079 odbiera direct INSERT refund claim. Bartek musi potwierdzić dokładne SHA wdrożonego PR #35, migracje, konfigurację operatora, historię faktur/zwrotów, wyścig dwóch sesji, ponowiony webhook, FK retencji i test mode. Stałe 23% VAT i korekty wymagają księgowego. Zewnętrzny refund/chargeback, trwały claim Checkout ponad godzinę i pełny transakcyjny outbox KSeF pozostają następnymi zadaniami; niepewną wysyłkę trzeba uzgadniać ręcznie.

## 2026-09-25 — widoczność niepewnych zwrotów i granica ID płatności Stripe (CYB-F03-29)

**Dyspozycja i stan wyjściowy:** Igor polecił kontynuować bez czekania na potwierdzenie migracji 00078 na db-1. Pracę wykonano w izolowanym worktree i na gałęzi codex/security-stripe-external-refunds, wychodząc z dokładnego HEAD PR #39: d10af4557d7b559cbc3a416e5ec74bcd42360e1d. 00078 pozostaje niepotwierdzona, więc traktujemy ją i zależną 00079 jako niewdrożone. Główny checkout i zmiany Igora pozostały nietknięte.

**Problem i dowody:** monitor zwrotów liczył tylko operacje processing starsze niż 15 minut; stan reconciliation_required, nadawany po niepewnym wyniku Stripe, mógł nigdy nie wzbudzić tego alarmu. Mapper opłaconej faktury pozwalał zapisać płatność bez PI/Charge, przez co późniejszy zewnętrzny refund lub spór nie miał pewnego klucza powiązania. Endpoint nadal pomija refundy i spory z panelu Stripe; dawny runbook błędnie obiecywał ich obsługę, alerty i automatyczne korekty. To wynik przeglądu kodu i dokumentacji Stripe, bez dowodu incydentu na produkcji.

**Zmiana lokalna:** commit fc4ee0d dodaje osobny licznik wszystkich reconciliation_required do istniejącego alarmu, zachowuje próg 15 minut dla processing i fail-closed przy błędzie zapytania. Opłacony invoice subskrypcji bez poprawnego legacy PI/Charge kończy się przed zapisem bazy stanem webhooka failed z kodem payment_reference_missing_or_invalid, przeznaczonym do ręcznego uzgodnienia; dwóch poprawnych referencji nie odrzuca automatycznie. [Runbook zwrotów i sporów](../runbooks/refund-and-disputes.md) opisuje rzeczywisty stan, a [plan zewnętrznych zwrotów i sporów](STRIPE-ZEWNETRZNE-ZWROTY-SPORY-PLAN-2026-09-25.md) wyznacza następny etap 00080, preflight i testy.

**Decyzja po niezależnym przeglądzie:** przechwytywanie refund/dispute przez oznaczenie każdego webhooka failed zostało sprawdzone lokalnie i wycofane przed commitem. RPC 00076 nie odzyskuje claimu failed, a monitor powtarzałby alarm także dla poprawnych zwrotów administracyjnych bez ścieżki zamknięcia. Bieżący commit nie subskrybuje nowych typów i nie tworzy pozoru automatycznej ochrony przed zewnętrznym zwrotem. Następny handler wymaga osobnego stanu sprawy, idempotentnego połączenia z admin refundem i kontrolowanego potwierdzenia po dowodach.

**Weryfikacja:** finalny kod: 166 plików / 2972 testy Vitest PASS z tymczasowym aliasem do lokalnego jsdom, 66 testów XML PASS, pełny TypeScript PASS z tymczasowym aliasem typów jsdom, pełny lint 0 błędów i 26 wcześniejszych ostrzeżeń; lint zmienionych TS bez ostrzeżeń. Next.js build --webpack PASS, 82/82 stron, w izolowanym worktree bez wywołań Stripe/PostgREST/KSeF; lokalny node_modules wskazywał Next 16.2.6, podczas gdy package.json deklaruje 16.3.4, więc dokładny build zależności ma potwierdzić CI. Gitleaks na commicie fc4ee0d: 1 commit, 0 trafień; kontrola diff PASS. Dwóch niezależnych recenzentów wykryło i doprowadziło do wycofania wadliwego wariantu kwarantanny.

**Status i granice:** kod, testy i dokumenty są lokalne. Nie wykonano SQL/migracji, żadnego odczytu produkcyjnej bazy ani Stripe, merge, wdrożenia czy subskrypcji nowych eventów. Nie udowodniono dostarczenia alarmu Slack/Sentry; trzeba przetestować rzeczywistą konfigurację i monitor. Jeśli endpoint Stripe ma format Basil+ bez starszych pól PI/Charge, nowy guard celowo zatrzyma opłacone faktury — to ryzyko dostępności wymagające preflight przed rolloutem. Wpis failed nie jest samoczynnie ponawiany; operator musi uzgodnić pełny Stripe ID, lokalną płatność, job i fakturę VAT. Webhook nadal nie blokuje automatycznie zewnętrznego zwrotu/sporu.

**Bartek — następny odbiór:** potwierdzić dokładny SHA webu/workera na Coolify oraz wykonanie migracji 00075–00079 na db-1, zwłaszcza 00078; bez tego nie wdrażać pakietów zależnych. Odczytowo sprawdzić event.api_version, reprezentatywne podpisane invoice.payment_succeeded, brakujące PI/Charge, historyczne refundy/spory i ustawienia subskrypcji webhooka. W Stripe test mode oraz na kopii bazy sprawdzić brak referencji i ręczne rozliczenie failed receipt, oba backendy jobów i faktyczne dotarcie alarmu do operatora. Do wdrożenia zewnętrznych zdarzeń wykonać transakcyjną bramę i procedurę zamknięcia z planu; korektę VAT/KSeF zatwierdza księgowy.

## 2026-09-25 — zewnętrzne zwroty i spory Stripe (CYB-F03-30)

**Dyspozycja, autor i stan wyjściowy:** Igor polecił kontynuować bezpieczeństwo. Codex przygotował pakiet na osobnej gałęzi codex/security-stripe-external-events od dokładnego HEAD PR #42, e5da18385cd3145a8564b047ab5543807b572bc6; kodowy commit b09739ca338568d23aa598bafd4f0817b963c656. Główny checkout i zmiany Igora pozostały nietknięte. 00078 jest tylko w release i nie ma dowodu wykonania na db-1, dlatego 00078 oraz zależne 00079/00080 traktujemy jako niewdrożone.

**Problem i źródło:** Podpisany endpoint pomijał refundy i spory z panelu Stripe, więc lokalny claim faktury/zwrotu nie widział zewnętrznej zmiany. Podpisane faktury w wariancie Acacia mogły zawierać rozwinięte PI/Charge, których mapper nie przyjmował. Statyczny przegląd i niezależna recenzja wykazały też wyścigi między nową sprawą, płatnością, fakturą VAT, administracyjnym zwrotem i ręcznym zamknięciem. Nie stwierdzono rzeczywistego incydentu na produkcji.

**Zmiana lokalna:** Endpoint odbiera refund.created/updated/failed, charge.refund.updated i pięć typów charge.dispute.*. Po weryfikacji podpisu i claimie pobiera aktualny Refund/Dispute po pełnym ID; jedna transakcyjna procedura zapisuje sprawę i wszystkie pełne referencje. Migracja 00080 dodaje RLS/granty, kwarantannę bez zgadywania tenanta, trwały hold dla VAT i kolejnego zwrotu, wspólne blokady referencji przed i po powstaniu lokalnej płatności, append-only historię review/reopen, ponowne otwarcie po zmianie dopasowania albo przepływie środków, oraz owner-only review z oczekiwanym event ID/status/count/payment ID. Poprawny lokalny refund może zostać oznaczony settled, ale hold historyczny pozostaje. Admin ponownie sprawdza hold tuż przed refunds.create; każdy wynik inny niż clear kończy się uzgodnieniem bez wywołania Stripe. Monitor liczy otwarte sprawy, a runbook podaje próby i kolejność wdrożenia. Sygnał charge.refunded nie jest obsługiwany, bo pojedynczy event może obejmować wiele częściowych zwrotów; okresowy sweep Stripe pozostaje do zrobienia.

**Weryfikacja:** 167 plików / 3009 dostępnych lokalnie testów Vitest PASS; dwa wcześniejsze testy UI wyłączono wyłącznie z powodu brakującego w tym worktree modułu jsdom. Wcześniej 66 testów XML PASS. Pełny lint: 0 błędów i 26 wcześniejszych ostrzeżeń. Lokalny Next build --webpack i jego TypeScript PASS, 82/82 stron; lokalny node_modules zawiera Next 16.2.6 mimo deklaracji 16.3.4, więc dokładny lockfile sprawdzi CI. Kontrola staged diff i statyczna kontrola delimitera SQL PASS. Niezależny przegląd po poprawkach nie znalazł dalszego konkretnego P1/P2. Nie wykonano SQL ani próby współbieżności dwóch sesji PostgreSQL.

**Stan i granice przed publikacją:** kodowy commit i dziennik były lokalne, bez merge, wykonania migracji, zmiany konfiguracji Stripe, odczytu produkcyjnej bazy ani wdrożenia. Stripe i baza nie mają wspólnej transakcji: zdarzenie dostarczone po wystawieniu VAT lub po końcowym preflightcie admina wymaga uzgodnienia; nie ma automatycznej korekty VAT. Finansowy receipt failed/processing bez trwałej sprawy sam nie zakłada hold; nie wolno resetować claimu w ciemno. Kod nie odtwarza historii Stripe ani nie dowodzi, że endpoint subskrybuje zdarzenia. Szczegóły i reakcja operatora są w [runbooku 00080](STRIPE-SPRAWY-FINANSOWE-ODBIOR-2026-09-25.md).

**Bartek — warunek odbioru:** potwierdzić SHA webu/workera w Coolify i faktyczne wykonanie 00075–00079 na db-1, zwłaszcza 00078; odczytowo uzgodnić historię płatności/refundów/sporów i format event.api_version; sprawdzić kopię oraz restore. Na kopii bazy przetestować kolejność dwóch sesji, granty, RLS, późną płatność i review CAS; w Stripe test mode sprawdzić pełny/częściowy zwrot, spór, kolejność webhooków, awarie i rzeczywiste alarmy. Dopiero po dowodach wdrożyć 00080 ze zgodnym webem/workerem, włączyć typy zdarzeń i nadzorować liczniki. Przy failed/processing bez sprawy wstrzymać automat do ręcznego porównania; korektę VAT zlecić księgowemu. Kolejny etap: audytowalne odzyskiwanie niepewnych receiptów oraz okresowy sweep Stripe.
...
## 2026-09-25 — odczytowy audyt luk spraw finansowych Stripe (CYB-F03-32)

**Dyspozycja i baza:** Igor polecił kontynuować bezpieczeństwo. Izolowana gałąź codex/security-stripe-gap-audit pierwotnie startowała z dokładnego HEAD roboczego PR #43, caa2382377ff6b438ad6d2c185bafec78815f48a; przed publikacją została ułożona nad PR #44, 4a7c35f0d7877cd15f482ed3676e5faabb279353. Commit 75b18df90a07e40a22e6bd5f57b06f998157f6bc zawiera kod, testy i [runbook audytu](STRIPE-LUKI-FINANSOWE-AUDYT-2026-09-25.md). Główny checkout pozostał nietknięty. Migracja 00078 ma niepotwierdzony stan na db-1, zatem zależne 00079/00080 traktujemy jako niewdrożone.

**Problem i granica:** PR #43 rejestruje dostarczone zwroty i spory, lecz nie odtwarza historycznych brakujących dostaw. Sam licznik webhooków nie wykrywa refundu/sporu, którego endpoint nigdy nie dostał. charge.refunded jest celowo pominięty, bo jeden event może reprezentować kilka częściowych zwrotów. Istniejący record_stripe_financial_case wymaga receiptu processing; bezpieczne automatyczne odzyskiwanie wymaga osobnego protokołu, nie wywołania tego RPC poza claimem.

**Zmiana:** lokalny skrypt operatorowy porównuje pełne re_/du_ z Stripe, utworzone w jawnym oknie 1–31 zakończonych dni UTC, z stripe_financial_cases. Wymaga --mode test/live zgodnego z prefiksem klucza; najpierw potwierdza dostępność tabeli 00080, potem stronicuje refundy/spory z twardym limitem 50 stron po 100 na typ. Dopasowanie lokalne działa po paczkach pełnych ID i stronicuje odpowiedzi bazy z kontrolą count, duplikatów i limitu. Osobno pokazuje wszystkie finansowe receipty failed oraz processing/retryable starsze niż 15 minut z całej dostępnej historii bazy — **ten fragment nie jest ograniczony wybranym oknem Stripe**. Każde ucięcie, niespójność, awaria źródła lub brak schematu daje incomplete, nigdy fałszywe zero. Wynik zawiera tylko liczniki i pełne identyfikatory Stripe/receiptu, bez payloadów, danych klienta i surowych błędów. Kod nie ma ścieżki zapisu, replay ani wywołania refundu.

**Weryfikacja lokalna:** 8 celowanych testów Vitest PASS, w tym paginacja Stripe/bazy, brak 00080 przed odczytem Stripe, niepewne receipty, niepełna odpowiedź bazy, limit 5000 i maskowanie błędu dostawcy. Lint trzech nowych plików TS PASS; TypeScript PASS w tymczasowej konfiguracji wyłączającej dwa wcześniejsze testy UI z brakującym lokalnie jsdom. Zwykły pnpm typecheck zatrzymał się wyłącznie na tych dwóch brakujących importach. Staged diff --check PASS. Zewnętrznych usług nie odpytywano, a kod lokalny korzysta ze wspólnych node_modules, nie pełnej instalacji dokładnego lockfile; wynik CI dla tej gałęzi nie istnieje.

**Granice i następny odbiór:** no_gaps_detected dotyczy tylko wskazanej pary klucz Stripe–URL bazy i przedziału dat; skrypt nie potrafi dowieść, że są to odpowiadające środowiska. Nie porównuje statusów starszych obiektów ani wszystkich kont Stripe. Bartek musi potwierdzić SHA webu/workera na Coolify, wykonanie 00075–00080, kopię/restore, test mode, endpoint, realne alerty i właściwy target bazy. Po wdrożeniu operator powinien wykonać ciągłe okna historyczne, porównać liczby z Dashboard i bezpiecznie rozliczyć wykryte ID oraz VAT/KSeF z księgowym. Codex nie wykonał SQL, migracji, testu z prawdziwym Stripe/PostgREST, merge ani wdrożenia. Publikacja tej gałęzi wymaga osobnego kroku; brak publicznego PR i CI.

## 2026-09-25 — trwały claim Checkout i potwierdzone alarmy krytyczne (CYB-F03-33)

**Dyspozycja i stan wyjściowy:** Igor polecił kontynuować zabezpieczanie billingu. Osobny worktree i gałąź codex/security-stripe-checkout-claim pierwotnie startowały z dokładnego HEAD roboczego PR #43, caa2382377ff6b438ad6d2c185bafec78815f48a; przed publikacją zostały ułożone nad lokalnym audytem d46966f6d28240e6dbfa8fe2eba028aab84b6c85 i PR #44. Główny checkout i zmiany użytkownika pozostały nietknięte. Wykonanie 00078 na db-1 pozostaje niepotwierdzone; dlatego 00078 oraz zależne 00079–00081 traktujemy jako niewdrożone.

**Problem i źródło:** dotychczasowy Checkout używał godzinowego klucza idempotencji. Po zmianie godziny albo utracie odpowiedzi dwa żądania tej samej firmy mogły utworzyć dwie otwarte, płatne sesje. Lokalna kontrola subskrypcji przed wywołaniem Stripe nie była atomowa względem równoległego żądania i webhooka. Osobno krytyczny alarm Slack mógł uznać brak konfiguracji, HTTP 4xx/5xx lub błąd sieci za dostarczenie i zapisać 30-minutowe wyciszenie przed potwierdzeniem wysyłki. Ustalenia pochodzą z przeglądu kodu, dokumentacji Stripe i testów hermetycznych; nie stwierdzono incydentu produkcyjnego.

**Zmiana lokalna:** po lokalnym ułożeniu stosu commit ed234d7fa641f341f1e749dbf04be420f6f3e14e obejmuje kod, migrację jako plik, testy i [runbook odbioru](STRIPE-CHECKOUT-CLAIM-ODBIOR-2026-09-25.md). Migracja 00081 definiuje pojedynczy nierozliczony claim per tenant, blokadę firmy wspólną z zapisem subskrypcji i RPC dostępne tylko dla service_role. Checkout utrwala losowy identyfikator próby przed Stripe i używa tego samego klucza idempotencji niezależnie od godziny; przed zwróceniem URL potwierdza Customer, tryb, status, przyszły termin ważności i zapis sesji. Niepewny wynik pozostaje w stanie wymagającym uzgodnienia, bez ślepej próby ponownej. Istniejące otwarte sesje Stripe zatrzymują nową próbę; ukończona sesja blokuje ją do sprawdzenia dokładnie powiązanej subskrypcji. Monitor liczy tworzenie starsze niż 15 minut, wszystkie stany uncertain/held oraz sesje open po terminie ważności + 15 minut. Krytyczny transport Slack wymaga potwierdzonego HTTP 2xx; deduplikacja następuje dopiero po dostarczeniu. Niekrytyczne powiadomienia pozostają odporne na błąd transportu.

**Weryfikacja:** 169 plików / 3023 dostępne testy Vitest PASS, w tym granice współbieżności, niepewny wynik Stripe, stare sesje i ponowienie alarmu po błędzie dostarczenia. Dwa wcześniejsze testy UI wyłączono z tego przebiegu wyłącznie dlatego, że modułu jsdom nie ma w lokalnym środowisku tego worktree. Pełny lint: 0 błędów i 26 wcześniejszych ostrzeżeń. TypeScript PASS w tymczasowej konfiguracji wyłączającej te same dwa testy; zwykły typecheck zgłasza jedynie brak jsdom w tych plikach. Lokalny Next.js build --webpack PASS, TypeScript PASS i 82/82 stron; lokalny node_modules wskazuje Next 16.2.6 wobec deklaracji 16.3.4, więc dokładne zależności potwierdzi CI. Kontrola staged diff PASS. Gitleaks 8.30.1 na dwóch nowych commitach: 0 trafień (skan ograniczony do ich diffu). Niezależny przegląd kodu, SQL i runbooka po poprawkach nie znalazł dalszego konkretnego P1/P2. Nie wykonano prawdziwej próby współbieżności na PostgreSQL ani wywołania Stripe/Slack.
